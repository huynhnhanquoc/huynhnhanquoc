...
- **Engine Package Test Suite:** **100% PASS** across all 60+ Go packages in [`engine/`](file:///d:/project/kitwork/engine).
...
### 2.1 Host Layer & Multi-Surface Manifest ([`app.kitwork.js`](file:///d:/project/kitwork/app.kitwork.js))
...
- **Value Representation ([`engine/value`](file:///d:/project/kitwork/engine/value)):** 24-byte tagged union / NaN-boxed `Value` structure holding primitive types, object pointers, string symbols, and lambda closures.
- **Energy Budgeting ([`engine/runtime/energy.go`](file:///d:/project/kitwork/engine/runtime/energy.go)):** Every opcode execution deducts energy units. Any program exceeding its budget is safely terminated without crashing the host process.
...
Kitwork embeds zero-dependency JIT engines inside [`engine/jit`](file:///d:/project/kitwork/engine/jit):
- **JIT CSS ([`engine/jit/css`](file:///d:/project/kitwork/engine/jit/css)):** Intercepts HTML streams and extracts active Tailwind utility classes to generate minimal, zero-unused CSS on the fly.
- **JIT Icons ([`engine/jit/icons`](file:///d:/project/kitwork/engine/jit/icons)):** Tabler Icons and Simple Icons mask engine generating inline SVG CSS masks (`<i class="icon-user">`).
- **KitJS Hydration ([`engine/jit/hydrate`](file:///d:/project/kitwork/engine/jit/hydrate)):** Delivers declarative HTML attribute reactivity (`data-kit-action`, scope frames) without client-side framework overhead.

### 2.5 Durable Background Workflows & Event Infrastructure ([`engine/work`](file:///d:/project/kitwork/engine/work))
